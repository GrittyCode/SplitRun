...
{
    // Loads the active theme's obstacle prefabs once at boot and keeps the handles resident for the
    // whole session, so the Game scene builds its pools synchronously with no per-run async gate.
    public sealed class AssetPreloadService : IDisposable
    {
        ...
        public AssetPreloadService(WorldThemeProfile theme)
        {
            _theme = theme;
        }
        ...
        // Loads run in parallel but registration follows the theme's declared order, so the seed-derived
        // variant index resolves to the same prefab on every client regardless of load completion order.
        private async UniTask LoadObstaclePrefabsAsync(CancellationToken ct)
        {
            ...
            foreach (FootprintPrefabs set in _theme.ObstaclePrefabs)
            {
                ...
                foreach (AssetReferenceGameObject reference in set.Prefabs)
                {
                    ...
                    // Loaded through Addressables rather than the reference's own cached handle so the
                    // shared theme SO never owns the session-scoped handles this service must release.
        ...
        private void Register(ObstacleFootprint footprint, GameObject prefab)
        {
            ...
            if (obstacle.Footprint != footprint)
                Debug.LogWarning($"[AssetPreloadService] '{prefab.name}' footprint {obstacle.Footprint} " +
                                 $"does not match its theme slot {footprint}.");
